...
`Definition（定义）`是指一个可以生成 Request Handler 的可调用对象，这个对象只有两个参数：一个是 `$handler` 就是上面说到的，业务逻辑的载体；另一个是 `$arguments`，是由用户提供的额外参数列表。当这个对象被调用时，在内部初始化 Request Handler 并返回。

这一概念的目的是帮助其他程序复用已有的 Request Handler 的功能，也许普通的使用者并不会涉及到编写一个 Definition，但了解它的作用对接下来的学习有所帮助。

## 如何初始化一个对象
延迟策略实际上是预定义一个生成可调用对象的过程，将这一过程推迟到第一次调用 `Request Handler` 时再进行。所以一个延迟策略需要准备的是：
1. 一个初始化策略，用来根据解析后得到的`类名称`和`方法名称`生成 `callable` 对象。
2. 一个调用策略，用来以某种方式调用这个生成的可调用对象。

下面的 demo 将运用 `Delay` 组件实现延迟策略，这里我们首先定义一个 Target 类，作为延迟策略的目标类：
```php
...
```
我们的目的是调用 `Target::say` 方法，得到生成的 response 对象。首先要考虑如何将类转化为 `callable` 对象，这里我希望将 `userName` 直接传递给构造方法，所以我们定义了一个的策略，代码如下：
```php
...
```
然后，我们观察 `Target::say` 方法，这个方法接受一句话，我们可以选择使用 `Vargs` 这个 `Request Handler` 作为调用策略，我们只需要将 `Vargs` 的 `Definition` 和额外的参数（指 `$word`）一并传入 `Delay` 组件即可完成构建：
```php
...
$app->handle($request);
```
我们可以看到，`Delay` 组件也是一个 `Request Handler`，只是它在内部与其他 Handler 做了连接。
...
## 利用 Delay 和容器实现控制反转
`依赖查找`是一种比较常见的控制反转模式，它相比近年来流行的依赖注入，性能稍好，并且构建简单，适用于中小型项目。我们利用 Delay 组件和 PSR container 即可实现这种模式。
```php
use ConstanzeStandard\Container\Container;
use ConstanzeStandard\Fluff\RequestHandler\Args;
use ConstanzeStandard\Fluff\RequestHandler\Delay;
...
...
```
我们在 Target 类初始化时传入 Container 对象，然后在类的内部进行依赖查找：
```php
...
## 支持依赖注入的初始化策略
Di 组件能够进行依赖注入，取决于其中的一个组件：`\ConstanzeStandard\DI\Manager`, 这个组件封装了一系列有关依赖注入的方法，`Di` 组件也将这一组件解耦出来，使你可以从外界传入，以便能够作用于架构的其他部分，或进行功能上的自定义。
...
...
```
我们利用 `Manager::instance` 方法初始化类，这样，Target 的构造方法也像普通方法一样支持依赖注入了。
...
...
```
这样，当 Target 类初始化完毕时，被标注的属性也将完成依赖注入。

有关依赖注入组件 `ConstanzeStandard\DI\Manager`，更详细的资料，包括参数列表和 Annotations 缓存的使用方法，请参阅 `constanze-standard/di` 的[官方文档](https://constanze-standard.github.io/di/)（推荐！使用 Annotations 最好开启缓存，否则消耗较大）.
